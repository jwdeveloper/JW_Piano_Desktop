Vue.component('midi-devices-component', {
    ...
    methods: {
      set_device(id)
      {
        ...
        if(device.active == true){
          device.handler.onmidimessage = this.on_midi
        }
        else
        {
          device.handler.onmidimessage = (e) =>{}
        }
      },
   },
    template: `
 ...
 function on_midi_success(midi, context) 
 {
    ...
    for(const device of midi.inputs.values())
    {
    ...
    for(const device of context.devices.values())
    {
      if(device.active == true)
      {
        device.handler.onmidimessage = on_midi_message
      }
